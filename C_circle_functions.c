...
// to compile on Cori use: cc -shared -Wl,-soname,C_circle_functions -fopenmp -o C_circle_functions.so -fPIC C_circle_functions.c
	...
	float lat[lat_dim][lon_dim], float radius, int lat_index_north, int lat_index_south, int lon_index_east, int lon_index_west, float total_lon_degrees)
{
	...
	for(int lon_index=lon_index_west; lon_index<lon_index_east; lon_index++) {  // longitude loop
		...
		for(int lat_index=lat_index_south; lat_index<lat_index_north; lat_index++) {  // latitude loop
			...
			for(int radius_index=-radius_gridpts; radius_index<radius_gridpts; radius_index++) {  // work way up circle
				lat_r = lat_index+radius_index;
				lat1 = lat[lat_index][lon_index];  // centre of circle
				lat2 = lat[lat_r][lon_index]; // vertical distance from circle centre
				...
				cos_lat = cos(0.0174533 * lat2);

				// longitude loop through circle
				for(int lon_circle_index=lon_index-lon_gridpts; lon_circle_index<lon_index+lon_gridpts; lon_circle_index++) {  // work across circle
					...
					for(int p_index=0; p_index<p_levels; p_index++) {
						tempv[p_index] = tempv[p_index] + (cos_lat*var[p_index][lat_r][cyclic_lon_index]);
						divider[p_index] = divider[p_index] + cos_lat;
