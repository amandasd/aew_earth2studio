...
# This function takes the variable and smooths it using a circular
# smoothing algorithm written in C for speed.
...
def filter_tracks(common_object,latlon_list,magnitude_list):
	...
	if len(latlon_list)>6 and sum(1 for x in magnitude_list if x<common_object.min_threshold)>3:
	...
	if latlon_list[-1][0] > 40. or latlon_list[-1][0] < -5.:
...
def advect_tracks(common_object, u850, u600, v850, v600, lat_lon_pair):
	...
	u_2d = (u850[:,:]+u600[:,:])/2. # take average of the 850 (index 0) and 600 (index 2) hPa levels
	v_2d = (v850[:,:]+v600[:,:])/2. # take average of the 850 (index 0) and 600 (index 2) hPa levels
